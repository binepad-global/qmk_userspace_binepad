...
enum {
    tapping_term_flag_permissive_hold         = 0x01,
    tapping_term_flag_hold_on_other_key_press = 0x02,
    tapping_term_flag_quick_tap_term          = 0x04,
    tapping_term_flag_retro_tapping           = 0x08
};

static void
auto_shift_timeout_apply(void) {
...
static void eeprom_settings_save(void) {
    for (size_t i = 0; i < sizeof(qmk_settings_t); ++i) {
        ...
        if (old_byte != new_byte) dynamic_keymap_set_qmk_settings(i, new_byte);
    }
}

static int eeprom_settings_get(const qmk_settings_proto_t *proto, void *setting, size_t maxsz) {
    uint16_t sz = pgm_read_word(&proto->sz);
    if (sz > maxsz) return -1;
...
static int eeprom_settings_set(const qmk_settings_proto_t *proto, const void *setting, size_t maxsz) {
    uint16_t sz = pgm_read_word(&proto->sz);
    if (pgm_read_word(&proto->sz) > maxsz) return -1;
...
static int magic_settings_get(const qmk_settings_proto_t *proto, void *setting, size_t maxsz) {
    ...
    if (maxsz < sizeof(flags)) return -1;
...
void qmk_settings_reset(void) {
    ...
    QS.tapping =
       ...
       tapping_term_flag_permissive_hold |
#endif
#ifdef HOLD_ON_OTHER_KEY_PRESS
       tapping_term_flag_hold_on_other_key_press |
#endif
#ifdef QUICK_TAP_TERM
       tapping_term_flag_quick_tap_term |
#endif
#ifdef RETRO_TAPPING
        tapping_term_flag_retro_tapping |
#endif
        0;
...
void qmk_settings_query(uint16_t qsid_gt, void *buffer, size_t sz) {
    ...
    for (size_t i = 0; i < sizeof(protos) / sizeof(*protos); ++i) {
        ...
        if (buffer_offset + sizeof(qsid) > sz) break;
...
static const qmk_settings_proto_t *find_setting(uint16_t qsid) {
    for (size_t i = 0; i < sizeof(protos) / sizeof(*protos); ++i)
        if (pgm_read_word(&protos[i].qsid) == qsid) return &protos[i];
    return NULL;
}
...
bool get_permissive_hold(uint16_t keycode, keyrecord_t *record) {
    return QS.tapping & tapping_term_flag_permissive_hold;
}

bool get_hold_on_other_key_press(uint16_t keycode, keyrecord_t *record) {
    return !(QS.tapping & tapping_term_flag_hold_on_other_key_press);
}

uint16_t get_quick_tap_term(uint16_t keycode, keyrecord_t *record) {
    if (QS.tapping & tapping_term_flag_quick_tap_term) {
        return 0;
    } else {
        return QS.tapping_term;
    }
...
bool get_retro_tapping(uint16_t keycode, keyrecord_t *record) {
    return QS.tapping & tapping_term_flag_retro_tapping;
}
